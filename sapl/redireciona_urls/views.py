...
class RedirecionaSAPLIndex(RedirectView):
    ...
    def get_redirect_url(self):
        ...
        try:
            self.logger.info("- Tentando obter url.")
            url = reverse(url_pattern)
        except NoReverseMatch as e:
            self.logger.error("- Erro ao obter url. " + str(e))
    ...
    def get_redirect_url(self):
        ...
        if pk_parlamentar:
            try:
                ...
                self.logger.info("- Tentando obter url correspondente.")
...
class RedirecionaNormasJuridicasTextoIntegral(RedirectView):
    ...
    def get_redirect_url(self, **kwargs):
        url = EMPTY_STRING
        try:
            self.logger.info("- Tentando obter NormaJuridica correspondente.")
            norma = NormaJuridica.objects.get(pk=kwargs['norma_id'])
            if norma:
                url = norma.texto_integral.url
        except Exception as e:
            self.logger.error("- Erro ao obter NormaJuridica. " + str(e))
            raise e
